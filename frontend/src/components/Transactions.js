...
const Transactions = ({ userId, accessToken }) => {
  ...
  const fetchTransactions = async (page = 1) => {
    ...
    if (!accessToken || !accessToken.startsWith('access-')) {
      console.error('Invalid access token format, cannot proceed with fetching transactions.');
      return;
    }

    try {
      setLoading(true); // Set loading state
      console.log('Fetching transactions with access token:', accessToken); // Log access token before use
  ...
  // Fetch transactions when component mounts or accessToken changes
  useEffect(() => {
    if (accessToken) {
      setTransactions([]); // Clear existing transactions
      fetchTransactions(1); // Load the first page of transactions
    }
  }, [accessToken, startDate, endDate, token]);
    ...
    <div className="min-h-screen bg-gray-900 p-8">
      ...
      <div className="flex space-x-8">
        ...
        <div className="w-full lg:w-3/5 space-y-6">
          ...
          {/* Transaction List */}
          <div className="bg-gray-800 p-4 rounded-lg shadow-lg h-96 max-h-96 overflow-y-scroll"> {/* Adjusted height and max-height */}
